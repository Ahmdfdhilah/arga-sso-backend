    ...
    async def CreateUser(
        ...
        async with await self._get_session() as session:
            try:
                ...
                if request.alias or request.gender or request.address or request.bio:
                    user.alias = request.alias if request.alias else user.alias
                    user.gender = request.gender if request.gender else user.gender
                    user.address = request.address if request.address else user.address
                    user.bio = request.bio if request.bio else user.bio
                    if request.HasField("date_of_birth"):
                        user.date_of_birth = request.date_of_birth.ToDatetime()
                    await session.flush()
