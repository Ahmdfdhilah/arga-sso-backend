"""Use case untuk Firebase authentication login dengan auto provider linking."""
...
class FirebaseLoginUseCase:
    """Use case untuk login menggunakan Firebase authentication dengan auto provider linking."""
    ...
    async def execute(
        ...
        Otomatis link provider jika user sudah terdaftar tapi belum punya Firebase provider.

        Args:
            request: FirebaseLoginRequest dengan firebase_token dan metadata lainnya
        ...
        if auth_provider:
            ...
            # Auto-link Firebase provider ke user yang sudah ada
            await self.auth_commands.create(
                user_id=user.id,
                provider=AuthProvider.FIREBASE.value,
                provider_user_id=firebase_user.uid,
            )
            logger.info(f"Auto-linked Firebase provider ke user: {user.id}")
